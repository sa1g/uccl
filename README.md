...
# UCCL with bare metal compilation support

Tested with `nvc++`, `gcc`. It has a code patch to support `nvc++` compilation.
...
| Target            | Code location      | Build path            | C++ target | Python target | Extra features            |
| ----------------- | ------------------ | --------------------- | ---------- | ------------- | ------------------------- |
|                   | `collective/afxdp` |                       | ❌         | ❌            |                           |
|                   | `collective/dpdk`  |                       | ❌         | ❌            |                           |
|                   | `collective/efa`   |                       | ❌         | ❌            |                           |
| `ccl_rdma_plugin` | `collective/rdma`  | `libnccl-net-uccl.so` | ✅         | ❌            |                           |
| `uccl_p2p_core`   | `p2p`              | `libuccl_p2p.so`      | ✅         | ❌            | DietGPU not supported yet |
|                   | `ep`               |                       | ❌         | ❌            |                           |
